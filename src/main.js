...
async function launchMinecraft({ username, version, memory, serverHost, serverPort, loader = 'vanilla', profileId = '', width = 1280, height = 720, fullscreen = false, customArgs = [], fastMode = true }) {
...
app.whenReady().then(() => {
  ...
  ipcMain.handle('minecraft:install-loader', async (_, { loader, version }) => { const java = await ensureJava(app.getPath('userData'), p => send('launcher:progress', { stage: 'java', progress: p, message: `Java ${p}%` })); const common = { root: minecraftRoot(), minecraftVersion: version, javaPath: java.path, onProgress: p => send('launcher:progress', { stage: 'loader', progress: p, message: `${loader.toUpperCase()} ${p}%` }) }; if (loader === 'fabric') return installFabric(common); if (loader === 'forge') return installForge(common); if (loader === 'neoforge') return installNeoForge(common); throw new Error('Loader ناشناخته است.') })
