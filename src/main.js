...
async function launchMinecraft({ username, version, memory, serverHost, serverPort, loader = 'vanilla', profileId = '', width = 1280, height = 720, fullscreen = false, customArgs = [], fastMode = true }) {
  ...
  const java = await ensureJava(app.getPath('userData'), (progress, received, total) => send('launcher:progress', { stage: 'java', progress, received, total, message: `در حال نصب Java... ${progress}%` }), version)
...
app.whenReady().then(() => {
  ...
  ipcMain.handle('app:open-external', (_, url) => { const value = String(url || ''); const parsed = new URL(value); if (!['https:', 'http:'].includes(parsed.protocol)) throw new Error('فقط لینک‌های HTTP/HTTPS مجاز هستند.'); return shell.openExternal(parsed.toString()) })
